export default class RevenueList {
  ...
  addTransaction(transaction) {
    let country_first_day = this.application_country_first_day.get(
      transaction.application_id,
    )
    if (!country_first_day) {
      country_first_day = new Map()
      this.application_country_first_day.set(
        transaction.application_id,
        country_first_day,
      )
    }
  ...
  *[Symbol.iterator]() {
    for (const [application_id, country_first_day] of this
      .application_country_first_day)
